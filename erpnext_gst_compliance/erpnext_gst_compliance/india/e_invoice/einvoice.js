	...
	frappe.ui.form.on(doctype, {
		async refresh(frm) {
			...
			const res = await frappe.call({
				method: 'erpnext_gst_compliance.erpnext_gst_compliance.india.e_invoice.utils.validate_eligibility',
				args: { doc: frm.doc }
			});
			...
			if (!irn && !__unsaved) {
				const action = () => {
					...
					frappe.call({
						method: 'erpnext_gst_compliance.erpnext_gst_compliance.india.e_invoice.utils.get_einvoice',
				...
				const action = () => {
					const d = new frappe.ui.Dialog({
						...
						primary_action: function() {
							...
							frappe.call({
								method: 'erpnext_gst_compliance.erpnext_gst_compliance.india.e_invoice.utils.cancel_irn',
								args: {
									doctype,
									docname: name,
									irn: irn,
				...
				const action = () => {
					const d = new frappe.ui.Dialog({
						...
						primary_action: function() {
							const data = d.get_values();
							frappe.call({
								method: 'erpnext_gst_compliance.erpnext_gst_compliance.e_invoice.utils.generate_eway_bill',
								args: {
									doctype,
									docname: name,
									irn,
			...
			if (irn && ewaybill && !irn_cancelled && !eway_bill_cancelled) {
				const action = () => {
					...
					frappe.confirm(
						...
						() => {
							frappe.call({
								method: "erpnext_gst_compliance.erpnext_gst_compliance.india.e_invoice.utils.cancel_eway_bill",
								args: { doctype, docname: name },
								freeze: true,
					...
					const action = () => {
						...
						const dialog = frappe.msgprint({
							...
							primary_action: {
								action: function() {
									frappe.call({
										method: 'erpnext_gst_compliance.erpnext_gst_compliance.india.e_invoice.utils.generate_qrcode',
										args: { doctype, docname: name },
										freeze: true,
...
const request_irn_generation = (frm) => {
	frappe.call({
		method: 'erpnext_gst_compliance.erpnext_gst_compliance.india.e_invoice.utils.generate_irn',
